...
if st.session_state.calculate:
    ...
    if affiliate_cpa > 0 and LTV < affiliate_cpa:
        st.warning("⚠️ **Warning:** Your Affiliate Marketing CAC ($" + f"{affiliate_cpa:,.2f}) "  
                  f"exceeds Customer LTV (${LTV:,.2f}). You'll lose money on every customer acquired through Affiliate Marketing channel. A lower Customer Acquisition Cost would help you to achieve profitability with this segment of users")
        ...
        try:
            ...
            if sem_cac_y1_val > 0 and LTV < sem_cac_y1_val:
                st.warning(f"⚠️ **Warning (Year 1):** Your SEM CAC (${sem_cac_y1_val:,.2f}) "  
                          f"exceeds Customer LTV (${LTV:,.2f}). You'll lose money on SEM-acquired customers. A lower SEM Cost per Click, or a higher Paid Traffic (SEM Traffic) conversion rate (Trial-To-Paid), would help you achieve profitability with this segment of users")
        except:
            pass
